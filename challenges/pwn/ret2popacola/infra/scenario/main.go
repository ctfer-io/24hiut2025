...
type Config struct {
	...
	Files              map[string]string `form:"files,omitempty"`
	IngressAnnotations map[string]string `form:"ingressAnnotations,omitempty"`
	IngressNamespace   string            `form:"ingressNamespace,omitempty"`
	IngressLabels      map[string]string `form:"ingressLabels,omitempty"`
		...
		cm, err := k8s.NewExposedMonopod(req.Ctx, "recipe-emp", &k8s.ExposedMonopodArgs{
			...
			Container: k8s.ContainerArgs{
				...
				Ports: func() k8s.PortBindingArray {
					out := make([]k8s.PortBindingInput, 0, len(conf.Ports))
				...
				Files: pulumi.ToStringMap(conf.Files),
			},
